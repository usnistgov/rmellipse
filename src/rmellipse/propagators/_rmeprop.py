...
class RMEProp(propagators.Propagator):
    ...
    def __init__(
        ...
        """
        self.settings = dict(
            montecarlo_sims=montecarlo_sims,
    ...
    def _get_unique_umech_id(
        *process_args: tuple[type], **process_kwargs: dict[type]
    ) -> list:
        """
        ...
        **process_kwargs : dict[type]
        ...
        """
        args_param_lists = [
            m.umech_id if uobjs.RMEMeas._if_quacks(m) else [] for m in process_args
        ]
        kwargs_param_lists = [
            m.umech_id if uobjs.RMEMeas._if_quacks(m) else []
            for m in process_kwargs.values()
        ]
    ...
    def _expand_umech_id_and_fill_nominal(
        ...
        if uobjs.RMEMeas._if_quacks(m) or xarray_skip_checks:
            ...
            intersections, comm1, comm2 = np.intersect1d(
                new_paramlocs, temp.umech_id, assume_unique=True, return_indices=True
            )
    ...
    def _run_linear(
        ...
        if sensitivity_analysis:
            ...
            cov_args = [
                RMEProp._expand_umech_id_and_fill_nominal(m, param_set)
                for m in process_args
            ]
            cov_kwargs = {
                k: RMEProp._expand_umech_id_and_fill_nominal(m, param_set)
                for k, m in process_kwargs.items()
            }
            ...
            if vectorize:
                ...
                if hasattr(cov_output, 'umech_id'):
                    for i, p in enumerate(param_set):
                        covargsi = [
                            a.sel(umech_id=p) if hasattr(a, 'umech_id') else a
                            for a in cov_args
                        ]
                        covkwi = {
                    ...
                    cov_output = cov_output.assign_coords(
                        umech_id=['nominal'] + param_set
                    )
                # if the output is a tuple, and contains RMEMeas objects
                elif isinstance(cov_output, tuple) and any(
                    [hasattr(co, 'umech_id') for co in cov_output]
                ):
                    for i, p in enumerate(param_set):
                        covargsi = [
                            a.sel(umech_id=p) if hasattr(a, 'umech_id') else a
                        ...
                        cov_output = [
                            ...
                            for co, po in zip(cov_output, proc_out)
                        ]
            ...
            cov_args = [
                m.cov.sel(umech_id=['nominal']) if uobjs.RMEMeas._if_quacks(m) else m
                for m in process_args
            ]
            cov_kwargs = {
                k: m.cov.sel(umech_id=['nominal']) if uobjs.RMEMeas._if_quacks(m) else m
                for k, m in process_kwargs.items()
            }
            cov_output = process_fcn(*cov_args, **cov_kwargs)
        else:
            ...
            def exp_nom(c):
                return c.expand_dims('umech_id', axis=0).assign_coords(
                    umech_id=['nominal']
                )

            if type(cov_output) is tuple:
                cov_output = tuple(
                    [exp_nom(c) if type(c) is xr.DataArray else c for c in cov_output]
                )
    ...
    def _run_montecarlo(
        ...
        if montecarlo_trials and RMEMeas_present:
            mc_args = [
                RMEProp._sample_distribution(montecarlo_trials, m) for m in process_args
            ]
            mc_kwargs = {
                k: RMEProp._sample_distribution(montecarlo_trials, m)
                for k, m in process_kwargs.items()
            }
    ...
    def _repack_linear_and_mc_outputs(
        ...
        if type(cov_output) is tuple:
            ...
            return tuple(
                [handle_output(cov, mc) for cov, mc in zip(cov_output, mc_output)]
            )
        else:
    ...
    def _get_new_categories(
        ...
        names = [
            ...
            if uobjs.RMEMeas._if_quacks(a) and a.covcats is not None
        ]
        names += [
            a.name
            for a in process_kwargs.values()
            if uobjs.RMEMeas._if_quacks(a) and a.covcats is not None
        ]
        cats = [
            a.covcats
                        ...
                        if not mismatch_ind.all():
                            ...
                            for i in range(0, len(newcats_bad)):
                                ...
                                msg = (
                                    ...
                                    + str(newcats_bad[i][badind].umech_id.values)
                                    + '\n'
                                )
                                msg += (
                                    'meas name: '
    ...
    def _get_new_covdofs(
        ...
        arr1 = [
            RMEProp._expand_umech_id_and_fill_nominal(m.covdofs, param_set, **kwargs)
            for m in process_args
            if uobjs.RMEMeas._if_quacks(m) and len(m.covdofs) > 0
        ]
        arr2 = [
            RMEProp._expand_umech_id_and_fill_nominal(m.covdofs, param_set, **kwargs)
            for m in process_kwargs.values()
        ...
        covdofs = xr.DataArray(
            covdofs, dims=('umech_id'), coords={'umech_id': param_set}
        )
    ...
    def handle_common_grid(
        self, process_args: tuple, process_kwargs: dict, dim: str, handle_method: str
    ) -> tuple[tuple, dict]:
        """
        ...
        if handle_method not in acceptable:
            raise Exception(
                'handling method "'
                + handle_method
                + '" not implemented. Accepable methods are '
                + str(acceptable)
            )
        new_args = None
        new_kwargs = None
        ...
        if handle_method == 'interp_common':
            ...
            def interp(x):
                return x.interp(
                    **self.settings['common_coords'],
                    kwargs=self.settings['interp_kwargs'],
                )

            new_args = [
                interp(a) if uobjs.RMEMeas._if_quacks(a) and hasattr(a.nom, dim) else a
                for a in process_args
            ]
            new_kwargs = {
                k: (
                    interp(a)
                    if uobjs.RMEMeas._if_quacks(a) and hasattr(a.nom, dim)
                    else a
                )
                for k, a in process_kwargs.items()
            }
        ...
        if handle_method == 'common':
            ...
            new_args = [
                sel(a) if uobjs.RMEMeas._if_quacks(a) and hasattr(a.nom, dim) else a
                for a in process_args
            ]
            new_kwargs = {
                k: sel(a) if uobjs.RMEMeas._if_quacks(a) and hasattr(a.nom, dim) else a
                for k, a in process_kwargs.items()
            }
    ...
    def propagate(self, fun):
        ...
        def fun_with_propagation(*args, **kwargs):
            ...
            if self.settings['verbose']:
                ...
                print(
                    '       linear runtime' + ':' + str(meta['linear_runtime']) + ' sec'
                )
                ...
                print(
                    ...
                    + ':'
                    + str(meta['recategorize_runtime'])
                    + ' sec'
                )
                print(
                    '       repack runtime' + ':' + str(meta['repack_runtime']) + ' sec'
                )
    ...
    def _generate_error_vectors(
        ...
        if generate_across_dim is not False:
            ...
            nom_shape = measurements[
                0
            ].nom.shape  # shape not including uncertainty mechanisms
        ...
        if len(unique) == 1 and unique[0] == 1:
            ...
            if n_single_values is not None and n_single_values != 1:
                ...
                if n_single_values < 1:
                    n_sv = (
                        np.where(np.cumsum(sv) / np.sum(sv) > n_single_values)[0][0] + 1
                    )
                else:
    ...
    def combine(
        ...
        *measurements: 'uobjs.RMEMeas',
        ...
        add_uuid: bool = True,
        combine_categories: dict[str] = {'Type': 'A'},
    ) -> 'uobjs.RMEMeas':
        """
        ...
        *measurements : RMEMeas
        ...
        err, typeA = RMEProp._generate_error_vectors(
            measurements_hg, **generate_error_kwargs
        )
        ...
        if self.settings['sensitivity']:
            ...
            try:
                ...
                def empty_row(n):
                    return xr.full_like(covcats[[0], ...], '').assign_coords(
                        {'umech_id': [n]}
                    )
                ...
                new_cols = [
                    empty_col(c)
                    for c in combine_categories
                    if c not in covcats.categories
                ]
                if len(new_cols) > 0:
        ...
        out = uobjs.RMEMeas(
            measurements_hg[0].name, avgcov, avgmc, covcats=covcats, covdofs=newcovdofs
        )
    ...
    def combine_across_dim(
        ...
        *measurements : RMEMeas
        ...
        # get PCA vectors and Type A uncertainties
        err, typeA = RMEProp._generate_error_vectors(
            ((measurement,)), **generate_error_kwargs
        )
        ...
        out = uobjs.RMEMeas(
            measurement.name, avgcov, avgmc, covcats=covcats, covdofs=newcovdofs
        )
