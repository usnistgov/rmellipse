...
DATASET_SAVEABLE = Union[
	bool, int, float, str, numpy.ndarray, NONETYPE
]  # bool, int, float, str,  don't work so I removed them for now (COLE) I don't think we use them anyways.
...
def save_list_saveable(
	group: GROUP, name: str, o: LIST_SAVEABLE, verbose: bool = False
) -> GROUP:
	"""
...
def save_dict_saveable(
	group: GROUP, name: str, o: DICT_SAVEABLE, verbose: bool = False
) -> GROUP:
	"""
...
def save_dataset_saveable(
	parent: GROUP, name: str, o: DATASET_SAVEABLE, verbose: bool = False
) -> DATASET:
	"""
...
def save_dataarray_saveable(
	parent: GROUP, name: str, o: DATAARRAY_SAVEABLE, verbose: bool = False
) -> DATASET:
	"""
...
def save_slice_saveable(
	group: GROUP, name: str, o: SLICE_SAVEABLE, verbose: bool = False
) -> GROUP:
	"""
...
def save_function_saveable(
	group: GROUP, name: str, o: SLICE_SAVEABLE, verbose: bool = True
) -> GROUP:
	"""
...
def load_object(
	...
	if save_type == 'LIST_SAVEABLE':
		...
		o = load_dataset_saveable(
			saved_object, o_class, vlen_object_encoding=vlen_object_encoding
		)
...
def load_list_saveable(
	group: GROUP, group_class: type, load_big_objects: bool = False
) -> LIST_SAVEABLE:
	"""
...
def load_dict_saveable(
	...
	if key_class_name in HUMAN_READABLE_KEY_TYPES:
		...
		for key in group.keys():
			...
			temp_values.append(
				load_object(group[key], load_big_objects=load_big_objects)
			)
...
def load_dataset_saveable(
	dataset: DATASET, dataset_class: type, vlen_object_encoding: type = str
) -> DATASET_SAVEABLE:
	"""
...
def load_dataarray_saveable(
	...
	if data_set is not NONETYPE:
		vals = _load_ndarray(
			data_set['values'], vlen_object_encoding=vlen_object_encoding
		)
		getting_dims = True
		dims = []
		i = 0
...
class GroupSaveable(GROUP_SAVEABLE):
	...
	@classmethod
	def load(
		cls, group: GROUP, parent: GROUP_SAVEABLE = None, load_big_objects: bool = False
	) -> GROUP_SAVEABLE:
		"""
	...
	def add_child(
		self, key: str = None, data: SAVEABLE = None, is_big_object: bool = False
	):
		"""
