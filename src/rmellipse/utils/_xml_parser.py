...
class MUFMeasParser:
    ...
    def open_data(
        ...
        if old_base_dir is not None:
            # change nominal path
            self.nominal_dict['location'] = self.nominal_dict['location'].replace(
        ...
        self.init_from_data(
            self.name, montecarlo_data, nominal_data, covariance_data, umech_id
        )
    ...
    def init_from_data(
        ...
        for i, data in enumerate(montecarlo_data):
            subdict = {'name': self.name + '_' + str(i), 'location': 'unknown'}
            self.montecarlo_dict.append(subdict)

        # nominal
        self.nominal_dict = {'location': '', 'name': self.name}
    ...
    def _parse_parameter_dict(self):
        ...
        names = [
            x.attrib['Text']
            for x in self.etree.findall(".//PerturbedSParams/Item/SubItem[@Index='0']")
        ]
        mechanisms = [
            x.attrib['Text']
            for x in self.etree.findall(".//PerturbedSParams/Item/SubItem[@Index='2']")
        ]
    ...
    def _parse_covariance_dict(self):
        ...
        locations = [
            x.attrib['Text']
            for x in self.etree.findall(".//PerturbedSParams/Item/SubItem[@Index='1']")
        ]
        mechanisms = [
            x.attrib['Text']
            for x in self.etree.findall(".//PerturbedSParams/Item/SubItem[@Index='2']")
        ...
        for index, name in enumerate(names):
            name_location_dictionary = {
                'name': name,
                'location': locations[index],
                'parameter_location': mechanisms[index],
            }
    ...
    def _parse_montecarlo_dict(self):
        ...
        names = [
            x.attrib['Text']
            for x in self.etree.findall(
                ".//MonteCarloPerturbedSParams/Item/SubItem[@Index='0']"
            )
        ]
        locations = [
            x.attrib['Text']
            for x in self.etree.findall(
                ".//MonteCarloPerturbedSParams/Item/SubItem[@Index='1']"
            )
        ]
        for index, name in enumerate(names):
    ...
    def _parse_nominal_dict(self):
        ...
        location = list(
            map(
                lambda x: x.attrib['Text'],
                self.etree.findall(".//MeasSParams/Item/SubItem[@Index='1']"),
            )
        )[0]
    ...
    def save_meas(self, output_file):
        ...
        MeasurementDocumentation_attrib = {
            'ControlType': 'System.Windows.Forms.TextBox',
            'ControlText': 'unknown',
            'FullName': 'Me_SplitContainer2__GroupBox5_MeasurementDocumentation',
        }

        CorrectedMeasurement = ET.Element(
            'CorrectedMeasurement', CorrectedMeasurement_attrib
        )
        MenuStripTextBoxes = ET.SubElement(CorrectedMeasurement, 'MenuStripTextBoxes')
        ET.SubElement(
            MenuStripTextBoxes, 'CreatedByUserName', CreatedByUserName_attrbib
        )
        ...
        ET.SubElement(
            MenuStripTextBoxes, 'CreatedByFirstFileName', CreatedByFirstFileName_attrib
        )
        ET.SubElement(
            MenuStripTextBoxes,
            'CreatedByUpdatedFileName',
            CreatedByUpdatedFileName_attrib,
        )
        ET.SubElement(
            MenuStripTextBoxes,
            'CreatedFromFirstFileName',
            CreatedFromFirstFileName_attrib,
        )
        ET.SubElement(
            MenuStripTextBoxes,
            'CreatedFromUpdatedFileName',
            CreatedFromUpdatedFileName_attrib,
        )
        ...
        ET.SubElement(
            Controls, 'MeasurementDocumentation', MeasurementDocumentation_attrib
        )

        # **********************************************************************
        # Monte-Carlo
        # **********************************************************************
        MonteCarloPerturbedSParams_attrib = {
            'ControlType': 'CustomFormControls.FLV_VariableDetailsList',
            'FullName': 'Me_SplitContainer2__GroupBox3_Panel2_MonteCarloPerturbedSParams',
            'Count': str(len(self.montecarlo_dict)),
        }

        MonteCarloPerturbedSParams = ET.SubElement(
            Controls, 'MonteCarloPerturbedSParams', MonteCarloPerturbedSParams_attrib
        )
        for i in range(len(self.montecarlo_dict)):
            ...
            SubItem0_attrib = {'Index': '0', 'Text': self.montecarlo_dict[i]['name']}
        ...
        # **********************************************************************
        MeasSParams_attrib = {
            'ControlType': 'CustomFormControls.FLV_FixedDetailsList',
        ...
        Item_attrib = {'Index': '0', 'Text': self.nominal_dict['name'], 'Count': '2'}

        SubItem0_attrib = {'Index': '0', 'Text': self.nominal_dict['name']}

        SubItem1_attrib = {'Index': '1', 'Text': self.nominal_dict['location']}
        ...
        # Covariance
        # **********************************************************************
        PerturbedSParams_attrib = {
            'ControlType': 'CustomFormControls.FLV_VariableDetailsListMeas',
            'FullName': 'Me_SplitContainer2__GroupBox1_Panel1_PerturbedSParams',
            'Count': str(len(self.covariance_dict)),
        }

        PerturbedSParams = ET.SubElement(
            Controls, 'PerturbedSParams', PerturbedSParams_attrib
        )
        for i in range(len(self.covariance_dict)):
            ...
            SubItem0_attrib = {'Index': '0', 'Text': self.covariance_dict[i]['name']}
