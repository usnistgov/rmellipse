...
def build_workflowtree(
	workflow_file: Path, project_settings: ProjectSettings
) -> WorkflowTree:
	"""
...
def validate_datapointers(
	datapointers: list[DataPointer], project_settings: ProjectSettings
):
	# if pnts, then just check that
	# the datasets claimed to exists actually exist
	cprint('DataSets', color=colors.HEADER)
...
def execute_concurrent_jobs(jobs: list[Job], group_name: Any):
	"""
		...
		for i, cj in enumerate(jobs):
			if not cj.thread.is_alive():
				...
				if cj.result['returncode'] == 0:
					msg += ''.join(
						cstr(f'  {symbols.CHECK} | {cj.name}\n', color=colors.OKGREEN)
					)
				else:
					msg += ''.join(
						cstr(f'  {symbols.BIGX} | {cj.name}\n', color=colors.FAIL)
					)
					failed.add(cj.name)
			else:
				msg += f'  {s} | {cj.name}\n'
...
if __name__ == '__main__':
	run(Path(r'tests/workflow-hello/hello'), Path(r'tests/workflow-hello'))
