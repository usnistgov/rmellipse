...
class ArchiveInterface(ABC):
	"""
	Interface for interacting through archives utilized by command line utitility functions.
		...
		def inner(*args, **kwargs):
			...
			if len(available_records) == 0:
				raise ReleaseRecordNotFoundError(
					f'{kwargs["title_versionless"]} with version {kwargs["version_expressions"]} not found.'
				)
	...
	def process_and_upload_blob(
		self, posix_rel_path: str, working_dir: Path
	) -> Tuple[str, int]:
		"""
...
class CDCSArchive(ArchiveInterface):
	...
	def __init__(self, host: str, user: str, password: str = None):
		...
		self.supports_repeat_releases = False
	...
	def download_blob(
		...
		update_dict : dict
		    ...
		    used to monitor the downlaod process when spun up
		    into threads.
		expected_size : int
