...
def format_release_verions(title: str, version: str):
...
def make_globs(directory: Path):
	...
	try:
		...
		ign_globs = [str(directory / g) for g in globs if g[0] != '#' and g[0] != '!']
		incl_globs = [str(directory / g[1:]) for g in globs if g[0] == '!']
	...
	try:
		with open(directory / '.rmeinclude', 'r') as f:
			globs = [line.strip() for line in f]
			globs = [str(directory / g) for g in globs if len(g) > 0]
		incl_globs += [str(directory / g) for g in globs if g[0] != '#']
	except FileNotFoundError:
		pass
	...
	for child_name in group:
		...
		if not matches_any(child_path, new_ign) or matches_any(child_path, new_incl):
			...
			if isinstance(child, h5py.Group):
				...
				map_hdf5(
					mapping[child_name], child, child_path, new_ign, new_incl, root=root
				)
...
def iter_blobable(
	...
	for name, child in dmap.items():
		if name[0] != '/':
			child_type = flowbals.DIRECTORY_ITEMS(
				child[flowbals.MAPPING_META_KEYS.ITEM.value]
			)
	...
	for name, item in map.items():
		if name[0] != '/':
			...
			if matches_any(path, show_only):
				item_type = flowbals.DIRECTORY_ITEMS(
					item[flowbals.MAPPING_META_KEYS.ITEM.value]
				)
				if level > 0:
				...
				print('')
				show_map(
					item, level=level + 2, show_attrs=show_attrs, show_only=show_only
				)


def folder_structure_from_project_map(project_map: dict, target_dir: Path):
	"""
	Build a folder structure based on a project map.
	...
	target_dir : Path
	    Target directory to recreate the mapping.
	"""
	for name, item in project_map.items():
		if name[0] != '/':
			item_type = flowbals.DIRECTORY_ITEMS(
				item[flowbals.MAPPING_META_KEYS.ITEM.value]
			)
			if item_type == flowbals.DIRECTORY_ITEMS.DIRECTORY:
				pathspec = target_dir / Path(
					item[flowbals.MAPPING_META_KEYS.PATHSPEC.value]
				)
				pathspec.mkdir(parents=True, exist_ok=True)
				folder_structure_from_project_map(item, target_dir)


def sym_link_folder_structure(src: Path, target: Path):
	"""
	Recreate a folder 1-1 with symbolic links to the original.

	Only the Files are symlinked, the folders are recreated inside
	the target.

	Parameters
	----------
	src : Path
	    Source folder. Items inside this folder will be linked to
	    inside target.
	target : Path
	    Target folder to place the symlinks.
	...
	for item in src.iterdir():
		...
		if item.is_dir():
			...
			sym_link_folder_structure(item, new_dir)
		else:
			(target / name).resolve().symlink_to(item.resolve())
