...
class Saveable(GroupSaveable):
    def __init__(self, name: str):
        GroupSaveable.__init__(
            self, name=name, parent=None, attrs={'name': name, 'is_big_object': False}
        )
        self.vals = [1, 2, 3]
    ...
    def __init__(self, parent=None, attrs=None):
        if attrs is None:
            attrs = {
                'name': 'i_am_bereft',
                'is_big_object': False,
                'my_metadata': 'filepathprobably',
            }
        GroupSaveable.__init__(self, parent=None, attrs=attrs)
    ...
    with h5.File(TEST_FILE, 'w') as f:
        ...
        for i, p in enumerate(prims):
            ...
            assert all(
                [
                    kp == kr and vp == vr
                    for (kp, vp), (kr, vr) in zip(p.items(), read.items())
                ]
            )
